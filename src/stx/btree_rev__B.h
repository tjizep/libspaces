	...
	* interior node sizes by assuming a cache line size of config::bytes_per_page bytes. */
	struct btree_traits
	{
		enum
		{
			...
			max_scan = 0,
			interior_mul = 1,
			keys_per_page = 64, ///192 is good for transactions, 384
			caches_per_page = 4,
			max_release = 8,
			version_reload
		};
	...
	private:
		...
		public:
			...
			inline int find_lower(key_compare key_less,const key_interpolator &interp, const node_type* node, const key_type& key) const {
				...
				if (last_found > 0 && (!key_less(node->get_key(last_found), key)) && key_less(node->get_key(last_found - 1), key)) {
					return last_found;
				}
				ui4 l = 0, h = o;
		...
		public:
			...
			inline const key_type *keys_d() const {
				return &_keys[0];
			}
			...
			void copy(int to, const surface_node* other, int start, int count) {
				...
				for (unsigned int slot = start; slot < count; ++slot)
				{
					...
					_keys[ni] = other->_keys[slot];
					_values[ni] = other->_values[slot];
				}
	...
	private:
		...
		*/
		...
		void split_surface_node(typename surface_node::ptr surface, key_type* _newkey, typename node::ptr &_newsurface)
		{
			...
			newsurface->copy(0,surface.rget(), mid, surface->get_occupants());
