...
namespace lua_XT{
	...
	/* Note: this library is for lua 5.1 which lacks __pairs and __ipairs 
	 * meta methods
	 ...
	 */
	static int luaXT_inextu (lua_State *L) {
		i4 t = 1;
		...
		lua_pushvalue(L, t);			
		lua_pushnumber(L, n);			
		lua_call(L, 2, 1);	// expects only one result 			
	...
	/* Requires luaXT_inext as upvalue 1 or input must have a library metamethod __intex */
	static int luaXT_ipairs (lua_State *L) {
		i8 n = luaL_optinteger(L, 2, 1) - 1;
		luaL_checkany(L, 1);		
		if (luaL_getmetafield(L, 1, "__intex")) {
			lua_pushcclosure(L, luaXT_inextu, 1);		// pops __intex and pushes closure
		}else{
			lua_pushvalue(L, lua_upvalueindex(1));		// this is a function	
		}
		lua_pushvalue(L, 1);
		lua_pushinteger(L, n);
		return 3;
