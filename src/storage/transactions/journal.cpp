...
extern nst::i64 get_store_journal_size();
...
static void log_journal(const std::string &name, const std::string& jtype, nst::u64 sa){
	nst::synchronized _llock(llock);

	creation << name << " : " << jtype << " : " << sa << std::endl;
}
#else
static void log_journal(const std::string &, const std::string&, nst::u64){
		...
		if(last_synch != sequence){
			...
			if(force || os::millis() - last_check > 3000){
				...
				if(jf.exists()){
					...
					set_store_journal_size( jsize );

					if(force || jsize > (nst::u64)get_store_journal_lower_max() ){
						...
						if(singles == participants.size()){
							...
							set_store_journal_size( jf.getSize() );
						}
					}
				}
			}
